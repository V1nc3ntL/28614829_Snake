...
*/
void
dispError (const char *fct, const char *msg, ...)
{
  va_list args;
  va_start (args, msg);
...
*/
void
dispDebug (const char *fct, int level, const char *msg, ...)
{
  if (debug >= level)
    {
...
*/
int
read_inbuf (const char *fct, char *buf, size_t nbuf)
{
  ...
  if (!length)
	...
	dispError (fct, "Cannot read message's length (server has failed?)");
      ...
      if (r != 1)
	dispError (fct, "Cannot read message's length (server has failed?)");
      dispDebug (fct, 3, "prepare to receive a message of length :%lu",
		 length);
    }
 ...
 */
void
sendString (const char *fct, const char *str, ...)
{
  va_list args;
  va_start (args, str);
  ...
  if (sockfd < 0)
    dispError (fct,
	       "The connection to the server is not established. Call 'connectToServer' before !");
  ...
  if (r > 0)
    dispError (fct,
	       "Acknowledgement message too long (sending:%s,receive:%s)",
	       str, buffer);

  if (strcmp (buffer, "OK"))
    dispError (fct,
	       "Error: The server does not acknowledge, but answered:\n%s",
	       buffer);
 ...
 */
void
connectToCGS (const char *fct, char *serverName, int port, char *name)
{
  ...
  dispDebug (fct, 2, "Initiate connection with %s (port: %d)", serverName,
	     port);
  ...
  if (connect (sockfd, (struct sockaddr *) &serv_addr, sizeof (serv_addr)) <
      0)
    dispError (fct, "Connection to the server '%s' on port %d impossible.",
	       serverName, port);
...
*/
void
closeCGSConnection (const char *fct)
{
  if (sockfd < 0)
    dispError (fct,
	       "The connection to the server is not established. Call 'connectToServer' before !");
 ...
 */
void
waitForGame (const char *fct, char *training, char *gameName, char *data)
{
  int r;
  if (training)
  ...
  do
    {
      ...
      if (r > 0)
	dispError (fct,
		   "Too long answer from 'WAIT_GAME' command (sending:%s)");
    }
 ...
 */
int
getGameData (const char *fct, char *data, size_t ndata)
{
 ...
 */
t_return_code
getCGSMove (const char *fct, char *move, size_t nmove)
{
 ...
 */
t_return_code
sendCGSMove (const char *fct, char *move)
{
  t_return_code result;
 ...
 */
void
sendCGSComment (const char *fct, char *comment)
{
